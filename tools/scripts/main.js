    ...
    $('.moves > p').on("keyup", function () { // main_gi: This sets all displayed text to the new description on contenteditable change
      if (this.previousSibling) {
        ...
        if (movename.startsWith("custom")) { // This is just so it works in exports hopefully
          ...
          if (MOVES[SMOVE[movename]].nobox) {DATA.custom[MOVES[SMOVE[movename]].id].nobox = MOVES[SMOVE[movename]].nobox} else {MOVES[SMOVE[movename]].nobox = "false"}
...
function makepiece(seed) { // Somehow I feel like this being in my life is a reference to me having played Layton Brothers several years ago (a character's name being Keelan Makepeace)
  ...
  if (r(randoms[25], 20) > 18) { // non generic passive (added on)
    ...
    rr(25);
    passives = passives.map(x => (x.length>0? x+"\\n":"") + nongenericpassives[passivepicked].replace(/\#\#/, r(randoms[25], 2, 6).toString() + `[+${r(randoms[26], 10, 30)}]`.replace(/\#/g, r(randoms[25], 2, 6).toString() + `[+${r(randoms[26], 1, 5)}]`))) // the "lose morale" number, something idiotic
...
function dealwithpiecename (passitoffto, extradata="") { // passitoffto is the function that #makeceo, #makefc, etc. use for the game specific piece
  ...
  let randomkocname = randarray(`kocbftn/ntfbcok/fnkobtc/awetalehu/jonny/Gum/GummyMan/sblmlmsg/mtgtcg/6aaaaaa/kfw1/mean_joe/flpenteros/#leaknewceo/#underminenewceo/fuolz/del10610791_1240/Randalf23/geklypip/nakari2/discorduser1/jdhfmxc`.split("/"))
...
var kocpositive = ["best piece ever made", `genius idea`, `never done before idea, really cool`, `this has never been done before`, `ive never seen this done before`, `wow this idea is good`, `wow this is the best piece ive seen so far`, `one of the best custom pieces`]
var wouldkiss = [`would kiss designer`, `i would kiss the designer`, `can i get autograph from the designer`, `even better than f3s pieces`, `so much better than most other piecs`, `beautiful design`, `great design`]
