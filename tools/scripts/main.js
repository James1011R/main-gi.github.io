    ...
    for (let i = 0; i < shuffledactions.length; i++) {
      ...
      for (let j = 0; j < actionassignments.length; j++) {
        if (actionassignments[j] == shuffledactions[i]) {
          if (shuffledactions[i] == 26-1 || shuffledactions[i] == 25-1) {differentsymmetry[j] = "none"; if (selectedgridspots[j][1] != 0) {selectedgridspots[j][0] == 0}} // action 26-1 = "Ability Target", which cannot be sideways ever.
